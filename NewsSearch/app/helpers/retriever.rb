...
def find_hit_list(invindex, docindex, query)
  ...
  # Check if there are any hits at all, if not return an empty hit_list
  if !hit_list || hit_list.size < 1
    return hit_list
  end
...
def compute_TFIDF(doc, query, invindex, docindex)
	...
	total_score = 0
	for term in query 
    term_frequencies = invindex[term]
    if !term_frequencies
      next
    end
		tf = invindex[term][doc]
		doc_stats = docindex[doc]
		ntf = tf / doc_stats.length
...
# takes a query as an array of keywords and a paths to the docindex file, the inverted 
# term index file and the stopwords file and returns a list of hits
def retrieve(doc_file, invindex_file, stopwords_file, query)
  query = query.split

  stop_words_hash = load_stopwords_file(stopwords_file)
  query = remove_stop_tokens(query, stop_words_hash)
  query = stem_tokens(query)
  # read in the index file produced by the crawler from Assignment 2 (mapping URLs to filenames).
  docindex=load_docindex(doc_file)

  # read in the inverted index produced by the indexer. 
  invindex=read_data(invindex_file)

  # find the hitlist
  hit_list = find_hit_list(invindex, docindex, query)

  # i = 0
  # while i < hit_list.length
  # 	(doc, score) = hit_list[i]
  # 	doc_stats = docindex[doc]
  #   if doc_stats.title == nil
  #     doc_stats.title = "nil"
  #   end
  # 	puts "score: " + 
  # 	score.to_s + "\nurl: " + 
